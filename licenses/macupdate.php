...
macupdate_log("");
macupdate_log("Processed $item_name order for $full_name <$payer_email>.");

if (transaction_id_exists($dbh, $txn_id)) { 
	macupdate_log("Transaction ID already used: $txn_id"); // no email for this
} else if (!$program) { 
	macupdate_error("Invalid program: $item_name");
} else {
	$subject = macupdate_subject($language_code);
	$message = macupdate_message($full_name, $program, $payer_email, $language_code);
	...
	macupdate_log("Emailed $payer_email.");
