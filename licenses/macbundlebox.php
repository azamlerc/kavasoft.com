...
if (!$program) { 
	...
	$subject = macbundlebox_subject($language_code);
	$message = macbundlebox_message($full_name, $program, $payer_email, $language_code);
	send_message("$full_name <$payer_email>", $subject, $message);
	
	$date_paid = date('Y-m-d');
	
	$result = save_license($dbh, $full_name, $payer_email, $program, null, null,
		$date_paid, $payment_amount, 'Mac Bundle Box', $language, $version, null, $txn_id);
