...
$item_name = "HyperImage 3"; // get_value('product');
$payment_amount = 25; // get_value('price');
$full_name = get_value('name');
$payer_email =  get_value('email');
$txn_id = "MF" . rand(100000, 999999);
...
if (!$program) { 
	...
	$subject = macfriendly_subject($language_code);
	$message = macfriendly_message($full_name, $program, $payer_email, $language_code);
	send_message("$full_name <$payer_email>", $subject, $message);
	
	$date_paid = date('Y-m-d');
	
	$result = save_license($dbh, $full_name, $payer_email, $program, null, null,
		$date_paid, $payment_amount, 'MacFriendly', $language, $version, null, $txn_id);
